import { StringHelpers } from "../util/string-helpers";
import { db } from "./db";
...
const doom = await db
  .insert(games)
  .values({ name: "doom", words: doomwords })
  .returning();

await db
  .insert(games)
  .values({ name: "gloom", words: gloom_words })
  .returning();

await db.insert(games).values({ name: "zoom", words: zoom_words }).returning();

await db
  .insert(games)
  .values({ name: "html", words: html_elements })
  .returning();

await db
  .insert(games)
  .values({ name: "camelBands", words: camel_case_bands })
  .returning();

await db
  .insert(games)
  .values({ name: "keba-bands", words: kebab_case_bands })
  .returning();

await db
  .insert(games)
  .values({ name: "snake_bands", words: snake_case_bands })
  .returning();

await db
  .insert(games)
  .values({ name: "ruby strings", words: ruby_strings })
  .returning();

await db
  .insert(games)
  .values({ name: "erotic fiction", words: erotic_fiction })
  .returning();

await db.insert(games).values({ name: "dates", words: dates }).returning();

await db
  .insert(games)
  .values({ name: "random character", words: random_character })
  .returning();

await db.insert(games).values({ name: "pbcb", words: pbcb }).returning();

await db
  .insert(games)
  .values({ name: "es6 practice", words: es6pract })
  .returning();

await db
  .insert(games)
  .values({ name: "higgz lyrics", words: higgz_lyrics })
  .returning();

await db.insert(games).values({ name: "ez words", words: ezwords }).returning();
